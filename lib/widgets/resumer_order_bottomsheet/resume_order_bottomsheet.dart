...
class ResumeSetButtonVisibility {
  bool showButton;
  String buttonLabel;

  ResumeSetButtonVisibility({
    required this.showButton,
    required this.buttonLabel,
  });
}

class ResumeSetVisible {
  bool isVisible;

  ResumeSetVisible({required this.isVisible});
}
...
class ResumeOrderBottomSheet extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    getIt<EventBus>().on<ResumeSetVisible>().listen((event) {
      resumeController.handleShowResume();
    });

    getIt<EventBus>().on<ResumeSetButtonVisibility>().listen((event) {
      ...
      children: [
        Observer(builder: (_) {
          return DraggableScrollableSheet(
            initialChildSize: resumeController.shouldHideResume ? 0 : 0.1,
            minChildSize: resumeController.shouldHideResume ? 0 : 0.1,
            maxChildSize: 0.9,
            snap: true,
                  ...
                  child: ButtonWidget(
                      ...
                      onTap: () async {
                        await pageController.animateToPage(1,
                            ...
                            curve: Curves.ease);
                        await draggableController.animateTo(0.1,
